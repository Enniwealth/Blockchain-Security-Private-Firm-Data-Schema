[
  {
    ...
    "description": "The TadleConnectors.claim function accepts a validator parameter from the caller without any authentication or signature verification. This validator address is treated identically to msg.sender for cooldown purposes via _updateClaimTimestamps. An attacker can call claim(token, victimAddress, level) with any address as the validator, forcing that victim to enter a 24-hour cooldown even though they never interacted with the protocol. By repeatedly calling claim with the victim as validator, the attacker can indefinitely prevent the victim from claiming their allocation.",
    "primary_file": {
